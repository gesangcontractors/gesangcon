          ...
          <div className="relative flex md:flex-wrap flex-nowrap border-2 border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md:g7 max-md:border-none max-md:rounded-none max-md:gap-3">
            {features.map(({ id, icon, caption, title, text, button }) => (
              ...
              >
                ...
                <p className="caption mt-10 mb-5 max-md:mb-6">{caption}</p>
                <h2 className="max-w-400 mb-7 h3 text-p4 max-md:mb-6 max-md:h5">
                  {title}
                </h2>
                <p className="mb-11 body-1 max-md:mb-8 max-md:body-3">{text}</p>

                {/* Video Component yang menggantikan Button */}
                <div className="w-full max-w">
                  <video
                    className="w-full rounded-lg border-2 border-s3 hover:border-s4 transition-all duration-300"
                    autoPlay
                    muted
                    loop
                    playsInline
            ...
            {/* <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
              ...
              {details.map(({ id, icon, title }) => (
                <li key={id} className="relative pt-16 px-4 pb-14">
                  ...
                  <div className="flex items-center justify-center mx-auto mb-3 border-2 border-s2 rounded-full hover:border-s4 transition-all duration-500 shadow-500 size-30">
                    <img
                      src={icon}
                      alt={title}
                      className="size-28 object-contain z-20"
                    />
                  </div>
